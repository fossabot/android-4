...
/**
 * Integration tests for location provider compatibility fixes
 * Tests the fixes we implemented for LocationManager.NETWORK_PROVIDER issues
 */
@RunWith(AndroidJUnit4.class)
@MediumTest
public class LocationProviderCompatibilityTest {

    @Rule
    public ActivityScenarioRule<NearestActivity> activityRule = 
        new ActivityScenarioRule<>(NearestActivity.class);
    ...
    public void testActivityStartsWithoutLocationProviderCrash() {
        // Test that NearestActivity starts without crashing due to 
    ...
    public void testActivityHandlesLocationPermissions() {
        ...
        // Activity should be stable and not crash
    ...
    @Test
    public void testLocationHeaderUpdatesWithoutCrash() {
        // Test that location header updates work with our provider fixes
        onView(withId(android.R.id.list))
            .check(matches(isDisplayed()));
            
        // Give time for location updates to potentially occur
        try {
            Thread.sleep(2000);
    ...
    public void testActivityHandlesLocationUpdateRequests() {
        ...
        activityRule.getScenario().onActivity(activity -> activity.onResume());
